...
**Расчётная дата этого синтетического набора:** `2026-10-01`. Движок использует её как дату среза для оценки будущих сессий; это не системные часы.
...
- После статуса `completed` мероприятие не повторяется, если у события не задано `repeatable: true`. Регулярный клуб задаёт это свойство в каталоге событий.
