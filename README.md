...
Скрипт создаёт локальный HR-токен в `.env` и выводит его один раз. Откройте [главную страницу](http://localhost:3000), введите токен через меню профиля, затем откройте [траекторию](http://localhost:3000/journey), HR-обзор на `/hr`, импорт на `/import` и схему API на [http://localhost:8000/docs](http://localhost:8000/docs). HR может выбрать любой синтетический профиль для демонстрации; токен сотрудника привязан к одному ID. Без настройки LLM рекомендации рассчитываются детерминированно.
