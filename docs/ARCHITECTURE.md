...
LLM не может добавлять кандидатов или обходить эти правила. Перестановка разрешена только внутри диапазона в 5 deterministic score points. Модель выбирает 3–4 допустимых кода доказательств для каждого кандидата; сервер проверяет их и формирует объяснение из фактов профиля, разрывов, истории и требований цели. Невалидный ответ, timeout или отсутствие ключа приводят к fallback. В outbound payload нет ФИО и `employee_id`; process-local cache ограничен по TTL и размеру и объединяет одинаковые параллельные запросы.
...
- API сопоставляет Bearer-токены из серверной конфигурации с ролью и employee ID. Employee-токен ограничен одним сотрудником; HR-токен даёт доступ к списку, аналитике и импорту.
